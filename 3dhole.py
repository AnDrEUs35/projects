...
def move_func(s, t):
    ...
    dzdt1 = v_z1
    dv_zdt1 = - G * m * z1 / (x1**2 + y1**2) **1.5 * A * (np.exp(B * np.sqrt(x1**2 + y1**2)))
...
def animate(i):
    ball1.set_data(solve_func(i, 'point')[0], solve_func(i, 'point')[1])
    ball1.set_3d_properties(solve_func(i)[2])
